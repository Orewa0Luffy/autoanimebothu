...
class AnimetsuScraper:
    ...
    def download_episode(self, url, quality="auto", name_override=None, season_override=None, ep_num_override=None):
        ...
        # Guard: binary must exist before we attempt any download
        if not self.binary_path or not self.binary_path.exists():
            if self.progress_queue: self.progress_queue.put({'error': 'N_m3u8DL-RE binary not found. Cannot download.'})
            return False
