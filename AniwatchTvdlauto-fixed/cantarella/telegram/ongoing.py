...
def fetch_schedule_list(source="aniwatch"):
    ...
    if source == "subsplease":
        from cantarella.scraper.subsplease import SubsPleaseScraper
        return SubsPleaseScraper().get_schedule()
...
def fetch_subsplease_recently_updated():
    from cantarella.scraper.subsplease import SubsPleaseScraper
    scraper  = SubsPleaseScraper()
    recent   = scraper.fetch_recently_updated(resolutions=("1080", "720"))

    # Include everything from the RSS — SubsPlease only lists releases
    # from the last ~24 hours so no extra time filter needed.
    # The processed-episodes DB prevents re-uploads.
    return recent
...
def fetch_animetsu_recently_updated():
    ...
    # Filter by freshness — use a 6-hour window to avoid missing episodes
    # when Animetsu delays updating the aired_at timestamp.
    # Items with NO aired_at are always included (we cannot know when they aired).
    ...
    for anime in recent_list:
        ...
        if aired_at:
            # Only skip if clearly older than 6 hours
            if (now_ms - aired_at) <= six_hours_ms:
                filtered_results.append(anime)
        else:
            # No timestamp — include it; the processed-episodes DB will
            # deduplicate so we never re-upload the same episode twice.
            filtered_results.append(anime)
...
async def check_and_download_ongoing(client: Client, chat_id: int):
    ...
    subsplease_scraper = None

    if active_source == "subsplease":
        from cantarella.scraper.subsplease import SubsPleaseScraper
        subsplease_scraper = SubsPleaseScraper()
        recent_animes  = await asyncio.to_thread(fetch_subsplease_recently_updated)
        downloader     = cantarellatvDownloader()
        scheduled_data = await asyncio.to_thread(fetch_schedule_list, source="subsplease")
    ...
    for idx, anime in enumerate(recent_animes):
        try:
            if active_source == "subsplease":
                # For SubsPlease each RSS item IS one episode — no need to fetch a list.
                # We treat the anime as having one latest episode (the RSS item itself).
                entries = [{
                    "title":     f"Episode {anime.get('ep_num', '1')}",
                    "url":       anime.get("url", ""),
                    "ep_number": str(anime.get("ep_num", "1")),
                    "ep_id":     anime.get("id", ""),
                    "_rss_title": anime.get("_rss_title", ""),
                }]
            elif active_source == "animetsu":
