...
async def _handle_download(client: Client, message, url, status_msg, is_playlist=False, quality="auto", chat_id=None, name_override=None, season_override=None, ep_num_override=None, source="auto", rss_title=None):
    if chat_id is None:
    ...
    async with download_semaphore:
        if progress_chat_id and hasattr(status_msg, "id"):
            try:
                ...
                pass
        return await __handle_download_internal(client, message, url, status_msg, is_playlist, quality, chat_id, progress_chat_id, name_override, season_override, ep_num_override, user_chat_id, user_progress_msg, source=source, rss_title=rss_title)


async def __handle_download_internal(client: Client, message, url, status_msg, is_playlist, quality, chat_id, progress_chat_id, name_override, season_override, ep_num_override, user_chat_id, user_progress_msg, source="auto", rss_title=None):
    progress_q = Queue()
    upload_q = Queue()

    # ── SubsPlease: use SubsPleaseScraper (aria2c) instead of cantarellatvDownloader ──
    if source == "subsplease" or (url.startswith("magnet:") or url.endswith(".torrent")):
        from cantarella.scraper.subsplease import SubsPleaseScraper
        downloader = SubsPleaseScraper(progress_queue=progress_q)
